...
class OpenIncidentForm(FormAction):
    ...
    def submit(
        ...
        if localmode:
            message = (
                f"Zostanie otwarty incydent z następującymi szczegółami \
                jeżeli Serwis Helpdesk zostanie podłączony;\n"
                f"email: {email};\n"
                f"tytuł: {incident_title};\n"
                f"opis problemu: {problem_description};\n"
            ...
            message = (
                f"Pomyślnie utworzyłem incydeny {incident_number} dla Ciebie. "
                f"Ktoś wkrótce się z Tobą skontaktuje."
            )
